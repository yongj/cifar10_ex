...
def train_nn():
    ...
    w_local3 = tf.Variable(tf.truncated_normal(shape=[dim,384], stddev=0.1))
    b_local3 = tf.Variable(tf.constant(0.1,shape=[384]))
    local3 = tf.nn.relu(tf.matmul(pool2_reshape,w_local3)+b_local3)
    
    # local 4
    w_local4 = tf.Variable(tf.truncated_normal([384,192], stddev=0.1))
    b_local4 = tf.Variable(tf.constant(0.1,shape=[192]))
    local4=tf.nn.relu(tf.matmul(local3,w_local4)+b_local4)
    
    # softmax
    w_softmax = tf.Variable(tf.truncated_normal([192,NUM_CLASSES], stddev=0.1))
    b_softmax = tf.Variable(tf.constant(0.1,shape=[NUM_CLASSES]))
