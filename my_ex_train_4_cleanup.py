...
def train_nn():
    ...
    train_writer = tf.summary.FileWriter('/tmp/cifar10_my_ex/train')
    test_writer = tf.summary.FileWriter('/tmp/cifar10_my_ex/test')
    ...
    with tf.Session() as sess:
        ...
        saver.restore(sess,'/tmp/cifar10_my_ex/cifar10_param.ckpt')
        ...
        for i in range(10000):
            ...
            _ = sess.run([train_step],feed_dict={x:train_images,labels:train_labels,bat_size:BAT_SIZE_TRANING})
            if (i%100==0):           # evaluate on test data every 100 steps 
                test_loss, test_acc, summary = sess.run([loss,accuracy,merged],feed_dict={x:test_images,labels:test_labels,bat_size:BAT_SIZE_TEST})
            ...
            if (i%10==0):           # evaluate on batch data every 20 steps
                output_loss,output_acc,summary = sess.run([loss,accuracy,merged],feed_dict={x:train_images,labels:train_labels,bat_size:BAT_SIZE_TRANING})
