...
def train_nn(restart=True):
    ...
    x = tf.placeholder(tf.float32,[None, HEIGHT_DISTORTED, WIDTH_DISTORTED, DEPTH])
    ...
    test_images,test_labels = my_ex_input.get_image_label_test_distorted()
    ...
    with tf.Session() as sess:
        ...
        for i in range(300000):
            train_images, train_labels = my_ex_input.get_image_label_batch_distorted(bat_size = BAT_SIZE_TRANING)
            _ = sess.run([train_step],feed_dict={x:train_images,labels:train_labels,bat_size:BAT_SIZE_TRANING,keep_prob:1.0,step_i:i})
            if (i%100==0):           # evaluate on test data every 100 steps 
                test_loss, test_acc, summary = sess.run([loss,accuracy,merged],feed_dict={x:test_images,labels:test_labels,bat_size:BAT_SIZE_TEST,keep_prob:1,step_i:i})
            ...
            if (i%10==0):           # evaluate on batch data every 20 steps
                output_loss = sess.run([loss],feed_dict={x:train_images,labels:train_labels,bat_size:BAT_SIZE_TRANING,keep_prob:1.0,step_i:i})
                output_acc,summary = sess.run([accuracy,merged],feed_dict={x:train_images,labels:train_labels,bat_size:BAT_SIZE_TRANING,keep_prob:1,step_i:i})
                #summary = sess.run([merged],feed_dict={x:train_images,labels:train_labels})
                train_writer.add_summary(summary,i)
                print('\n step %d, loss: %f, batch accuracy:%f' % (i,output_loss[0],output_acc))
            if (i%1000==-1):      # send me an email every 100 steps
                message = format('Model 3 update => training step: %d, batch accuracy: %.2f, test accuracy: %.2f' % (i,output_loss[0],test_acc))
                my_ex_send_email.send_email('Accuracy Update for Model 3',message)
        train_writer.close()
        test_writer.close()
        


train_nn(restart=False)
